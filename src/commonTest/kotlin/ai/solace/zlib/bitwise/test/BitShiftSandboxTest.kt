...
class BitShiftSandboxTest {
            ...
            for (value in testValues) {
                for (shift in testShifts) {
                    if (shift < bitWidth) {
                        ...
                        assertEquals(nativeLeft.value, arithmeticLeft.value, 
                        ...
                        assertEquals(nativeUnsigned.value, arithmeticUnsigned.value,
    ...
    fun testHistoricalBitPatterns() {
        ...
        for ((value, shift, expected) in testCases) {
            ...
            assertEquals(expected, nativeResult.value, 
                "Native: Failed for 0x${value.toString(16)} << $shift")
            assertEquals(expected, arithmeticResult.value, 
                "Arithmetic: Failed for 0x${value.toString(16)} << $shift")
            assertEquals(nativeResult.value, arithmeticResult.value,
    ...
    fun testBitShiftEngineIntegration() {
        ...
        fun simpleHash(data: ByteArray, useEngine: Boolean = true): Long {
            var hash = 1L
            for (byte in data) {
        ...
        assertEquals(hashWithNative, hashWithEngine, 
